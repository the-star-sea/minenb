...
class MineRLAgent:
    ...
    def evaluate(self,state,action,hidden):
        pi_distribution, v_prediction,d_prediction, new_agent_state = self.policy.get_output_for_observation(
            state,
            hidden,
            self._dummy_first
        )
        log_prob = self.policy.get_logprob_of_action(pi_distribution, action)
        entropy=self.policy.pi_head.entropy(pi_distribution)
        return v_prediction,d_prediction,log_prob,entropy
# class Memory:
#     def __init__(self):
#
#         self.actions = []
